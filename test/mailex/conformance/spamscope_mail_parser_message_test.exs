defmodule Mailex.Conformance.SpamScopeMailParserMessageTest do
  @moduledoc """
  Conformance tests using SpamScope/mail-parser's .eml fixtures.

  These tests validate Mailex's message parsing against real-world email fixtures
  from the SpamScope mail-parser Python library. The fixtures cover:

  - Standard emails with various encodings (UTF-8, Cyrillic, French, etc.)
  - Multipart messages (mixed, alternative, related)
  - Attachments (single, multiple, nested)
  - Character encoding edge cases (GB2312, base64, quoted-printable)
  - Malformed emails with RFC violations
  ...
  @cases TestFixtures.load_spamscope_cases!()
  @deviations TestFixtures.load_spamscope_deviations!()
  @expected_file_count 21

  describe "fixture completeness" do
    test "all #{@expected_file_count} .eml files are present" do
      files = TestFixtures.list_spamscope_eml_files!()
  ...
  defp assert_message_expectations(message, expect, deviation) do
    # Skip subject/from assertions if deviation indicates encoding issues
    skip_header_assertions = deviation && deviation[:behavior] == :different_output

    if subject = expect[:subject] do
      unless skip_header_assertions do
    ...
    if from = expect[:from] do
      unless skip_header_assertions do
        assert message.headers["from"] == from,
               "Expected from #{inspect(from)}, got #{inspect(message.headers["from"])}"
      end
    end
